...
export default function SmartProductSearch({ onProductData, categories = [] }) {
  ...
  return (
    <div ref={containerRef} className="relative bg-white/70 backdrop-blur-xl rounded-3xl p-6 overflow-visible border border-white/50 shadow-[0_2px_20px_-6px_rgba(0,0,0,0.06)]">
      {/* Decorative elements */}
      ...
      <div className="relative space-y-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-[#1d1d1f] font-semibold text-[15px]">Cadastro Inteligente</h2>
            <p className="text-[#86868b] text-[12px]">Digite o nome do produto e a IA preenche tudo automaticamente</p>
          </div>
        </div>

        <div className="relative">
          <Search className="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-[#86868b] z-10" />
          <input
            type="text"
            value={query}
            ...
            disabled={loading}
            className="w-full h-14 pl-12 pr-36 bg-[#f5f5f7]/80 backdrop-blur-md border border-black/[0.06] rounded-2xl text-[#1d1d1f] text-[15px] placeholder:text-[#86868b] focus:outline-none focus:ring-2 focus:ring-violet-500/30 focus:border-violet-500/20 transition-all disabled:opacity-50"
          ...
          {showDropdown && !loading && query.trim().length > 0 && hasResults && (
            <div className="absolute top-full left-0 right-0 mt-2 bg-white/95 backdrop-blur-2xl border border-black/[0.06] rounded-2xl overflow-hidden shadow-2xl z-50 max-h-80 overflow-y-auto">
              {Object.entries(grouped).map(([family, models]) => (
                <div key={family}>
                  {/* Family header */}
                  <div className="px-4 pt-3 pb-1.5 flex items-center gap-2">
                    <span className="text-[10px] font-bold text-[#86868b] uppercase tracking-widest">{family}</span>
                  ...
                  {models.map((model) => {
                    ...
                    return (
                      <button
                        ...
                        className="w-full text-left px-4 py-2.5 text-[13px] text-[#6e6e73] hover:bg-[#f5f5f7] hover:text-[#1d1d1f] transition-colors flex items-center gap-3 group"
                      >
                        <span className="w-7 h-7 bg-[#f5f5f7] group-hover:bg-violet-500/15 rounded-lg flex items-center justify-center flex-shrink-0 transition-colors">
                          <Sparkles className="w-3 h-3 text-violet-500" />
                        </span>
                        <span className="flex-1 truncate">
                          {idx >= 0 ? (
                            <>{before}<span className="text-[#1d1d1f] font-semibold">{match}</span>{after}</>
                          ) : model}
                        </span>
                        <ChevronRight className="w-3.5 h-3.5 text-[#d2d2d7] group-hover:text-[#86868b] flex-shrink-0 transition-colors" />
                      </button>
                    );
                  })}
                </div>
          ...
          {/* No results hint */}
          {showDropdown && !loading && query.trim().length > 0 && !hasResults && (
            <div className="absolute top-full left-0 right-0 mt-2 bg-white/95 backdrop-blur-2xl border border-black/[0.06] rounded-2xl overflow-hidden shadow-2xl z-50">
              <div className="px-4 py-4 text-center space-y-1">
                <p className="text-[13px] text-[#86868b]">Nenhum modelo encontrado para "{query}"</p>
                <p className="text-[11px] text-[#b0b0b5]">Pressione <kbd className="px-1.5 py-0.5 bg-[#f5f5f7] rounded text-[10px] text-[#86868b]">Enter</kbd> para buscar com IA mesmo assim</p>
              </div>
            </div>
          )}
        </div>
        ...
        {loading && (
          <div className="flex items-center gap-2 text-[#86868b] text-[12px]">
