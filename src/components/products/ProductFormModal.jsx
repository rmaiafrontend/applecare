...
const inputClass = "h-10 rounded-xl text-[13px] border-black/[0.06] bg-[#fafafa] focus:bg-white transition-colors";
...
export default function ProductFormModal({ open, onOpenChange, editProduct, categories = [], tags = [] }) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-[680px] p-0 gap-0 rounded-2xl overflow-hidden max-h-[92vh] border-black/[0.06]">
        {/* Header */}
        <div className="flex items-center justify-between px-6 h-14 border-b border-black/[0.05] bg-white sticky top-0 z-10">
          <div className="flex items-center gap-3">
            <div className={`w-8 h-8 rounded-xl flex items-center justify-center ${isEditing ? "bg-blue-50" : "bg-emerald-50"}`}>
              <Package className={`w-4 h-4 ${isEditing ? "text-blue-500" : "text-emerald-500"}`} strokeWidth={2} />
            </div>
            <div>
              <h2 className="text-[14px] font-semibold text-[#1d1d1f] leading-tight">
                {isEditing ? "Editar Produto" : "Novo Produto"}
              </h2>
              {isEditing && (
                <p className="text-[10px] text-[#86868b] leading-tight">{editProduct.product_id}</p>
              )}
            </div>
          </div>
        ...
        <ScrollArea className="max-h-[calc(92vh-56px)]">
          <div className="p-6 space-y-6">
            ...
            <AnimatePresence>
              {aiFilled && (
                <motion.div
                  ...
                  className="bg-emerald-50 border border-emerald-100 rounded-xl px-4 py-3 flex items-center gap-3 overflow-hidden"
                >
                  <div className="w-6 h-6 bg-emerald-500/15 rounded-lg flex items-center justify-center flex-shrink-0">
                    <Check className="w-3 h-3 text-emerald-600" />
                  </div>
                  <p className="text-[12px] font-medium text-emerald-700">Dados preenchidos automaticamente pela IA</p>
                </motion.div>
              )}
            </AnimatePresence>
            ...
            <div className="flex items-center gap-3">
              ...
              <span className="text-[10px] font-semibold text-[#b0b0b5] uppercase tracking-widest">Dados do produto</span>
              <div className="h-px flex-1 bg-black/[0.04]" />
            </div>
            ...
            <FormSection title="Informações" icon={Info} accent="#007aff">
              ...
              <FieldGroup label="Descrição">
                <Textarea value={form.description} onChange={e => updateField("description", e.target.value)} placeholder="Descrição comercial do produto..." className="rounded-xl text-[13px] min-h-[80px] border-black/[0.06] bg-[#fafafa] focus:bg-white transition-colors resize-none" />
              </FieldGroup>
            </FormSection>
            ...
            <FormSection title="Imagens" icon={Image} accent="#ff9500">
              <div className="flex flex-wrap gap-2.5">
                {form.images.map((img, i) => (
                  <div key={i} className="relative w-[72px] h-[72px] rounded-xl overflow-hidden bg-[#f5f5f7] ring-1 ring-black/[0.06] group">
                    ...
                    {i === 0 && (
                      <span className="absolute bottom-1 left-1 text-[8px] font-bold bg-white/90 text-[#1d1d1f] px-1.5 py-0.5 rounded">Principal</span>
                    )}
                  </div>
                ))}
                <label className="w-[72px] h-[72px] rounded-xl border-2 border-dashed border-black/[0.08] flex flex-col items-center justify-center cursor-pointer hover:border-[#007aff]/40 hover:bg-blue-50/30 transition-all gap-1">
                  <Upload className="w-4 h-4 text-[#b0b0b5]" strokeWidth={1.8} />
                  <span className="text-[9px] text-[#b0b0b5] font-medium">Upload</span>
                  <input type="file" accept="image/*" className="hidden" onChange={handleFileUpload} />
                </label>
              </div>
              <div className="flex gap-2">
                <Input value={newImageUrl} onChange={e => setNewImageUrl(e.target.value)} placeholder="Ou cole a URL da imagem..." className="h-9 rounded-xl text-[12px] border-black/[0.06] bg-[#fafafa] focus:bg-white" />
                <button onClick={addImage} disabled={!newImageUrl} className="h-9 px-3.5 bg-[#f5f5f7] hover:bg-[#e8e8ed] text-[#1d1d1f] rounded-lg text-[11px] font-semibold transition-colors disabled:opacity-30">
                  Adicionar
                </button>
              </div>
            </FormSection>
            ...
            <FormSection title="Especificações" icon={Cpu} accent="#5856d6">
              {form.specs.length > 0 && (
                <div className="grid grid-cols-2 gap-2">
                  {form.specs.map((spec, i) => (
                    <div key={i} className="flex items-center gap-2 bg-[#fafafa] rounded-lg px-3 py-2 group border border-black/[0.03]">
                      <div className="flex-1 min-w-0">
                        <p className="text-[10px] text-[#86868b] font-medium leading-tight">{spec.label}</p>
                        <p className="text-[12px] text-[#1d1d1f] font-semibold truncate">{spec.value}</p>
                      </div>
                      <button onClick={() => removeSpec(i)} className="opacity-0 group-hover:opacity-100 transition-opacity flex-shrink-0 w-5 h-5 rounded flex items-center justify-center hover:bg-red-50">
              ...
              <div className="flex gap-2">
                <Input value={newSpecLabel} onChange={e => setNewSpecLabel(e.target.value)} placeholder="Label (ex: Tela)" className="h-9 rounded-lg text-[12px] border-black/[0.06] bg-[#fafafa] focus:bg-white" />
                <Input value={newSpecValue} onChange={e => setNewSpecValue(e.target.value)} placeholder="Valor (ex: 6.7 polegadas)" className="h-9 rounded-lg text-[12px] border-black/[0.06] bg-[#fafafa] focus:bg-white"
                  onKeyDown={e => { if (e.key === "Enter") addSpec(); }}
                />
                <button onClick={addSpec} disabled={!newSpecLabel || !newSpecValue} className="h-9 w-9 bg-[#f5f5f7] hover:bg-[#e8e8ed] rounded-lg flex items-center justify-center transition-colors disabled:opacity-30 flex-shrink-0 border border-black/[0.04]">
            ...
            <FormSection title="Tags" icon={Tags} accent="#ff2d55">
              {form.tags.length > 0 && (
                <div className="flex flex-wrap gap-1.5">
                  {form.tags.map(tag => (
                    <span key={tag} className="inline-flex items-center gap-1.5 text-[11px] font-medium bg-[#f5f5f7] text-[#1d1d1f] pl-2.5 pr-1.5 py-1 rounded-lg group border border-black/[0.03]">
                      {tag}
                      <button onClick={() => removeTag(tag)} className="w-4 h-4 rounded flex items-center justify-center opacity-40 group-hover:opacity-100 hover:bg-red-50 transition-all">
              ...
              <div className="flex gap-2">
                <Select value="" onValueChange={v => { if (!form.tags.includes(v)) updateField("tags", [...form.tags, v]); }}>
                  <SelectTrigger className="h-9 rounded-lg text-[12px] border-black/[0.06] bg-[#fafafa]"><SelectValue placeholder="Selecionar tag..." /></SelectTrigger>
                  <SelectContent>
                    {tags.filter(t => !form.tags.includes(t.slug)).map(t => <SelectItem key={t.id} value={t.slug}>{t.name}</SelectItem>)}
                  </SelectContent>
                </Select>
                <Input value={newTag} onChange={e => setNewTag(e.target.value)} placeholder="Ou digite..." className="h-9 rounded-lg text-[12px] border-black/[0.06] bg-[#fafafa] focus:bg-white" onKeyDown={e => { if (e.key === "Enter") addTag(); }} />
                <button onClick={addTag} disabled={!newTag} className="h-9 px-3.5 bg-[#f5f5f7] hover:bg-[#e8e8ed] text-[#1d1d1f] rounded-lg text-[11px] font-semibold transition-colors disabled:opacity-30 flex-shrink-0">
                  Add
                </button>
              </div>
            </FormSection>
            ...
            <FormSection title="Configurações" icon={Settings} accent="#636366">
              <div className="bg-[#fafafa] rounded-xl border border-black/[0.04] divide-y divide-black/[0.04] overflow-hidden">
                {[
                  { key: "express_delivery", label: "Entrega Express", desc: "Disponível para entrega em 1h", icon: Zap, color: "text-violet-500" },
                  { key: "is_featured", label: "Produto em Destaque", desc: "Exibir na seção de destaques", icon: Star, color: "text-amber-500" },
                  { key: "is_active", label: "Produto Ativo", desc: "Visível no catálogo público", icon: Eye, color: "text-emerald-500" },
                ].map(item => (
